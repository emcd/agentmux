...
#[test]
fn the_probe_report_says_how_keys_arrive_and_not_what_they_do() {
    // The report's subject is delivery: whether a modified Enter reaches the
    // TUI distinctly. It once ended by naming the chord that inserts a newline
    // regardless of the outcome, which made this a second place a binding was
    // written down and would have gone false the moment the row moved. That
    // line is now generated by the help renderer from the table, and the
    // portability claim it makes is asserted there, where the chord it prints
    // can be compared against the one the table declares.
    for enhancement in [
        ...
        assert!(
            !lines.iter().any(|line| line.contains("Ctrl+")),
            "{enhancement:?} report names a chord: {lines:?}"
        );
    }
}
