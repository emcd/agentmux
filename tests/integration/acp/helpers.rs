...
/// RAII guard for `acp::*` integration tests' `TempDir`.
///
/// Each `acp::*` test creates a new `TempDir` with `flat_bundle_paths` where
/// `runtime_directory == TempDir.path()`, and `startup_bundle` spawns 2
/// `acp_stub` workers (alpha + bravo) as children of the nextest worker.
/// Without cleanup, the `AsyncDeliveryRegistry` retains entries for deleted
/// temp dirs and the nextest worker's `acp_stub` children are never reaped,
/// leaking 3 per run on the green path. This guard removes the registry
/// entries and kills the `acp_stub` children via `acp_child_pids.txt` on drop.
