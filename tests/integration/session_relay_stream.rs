...
// === Cross-relay sender attribution: on_behalf_of (add-cross-relay-sender-attribution) ===
//
// on_behalf_of is an intermediary-asserted origin subject: advisory, single-hop,
// and never an authorization input. These exercise the three guarantees end to
// end — the origin relay stamps it from its verified requester, the receiving
// relay honors it only from a peer-relay (ingress) requester and surfaces it in
// the delivered envelope, and a self-asserting non-relay requester's value is
// dropped.
