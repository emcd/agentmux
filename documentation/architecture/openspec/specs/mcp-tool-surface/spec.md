...
`send` SHALL NOT accept any transport-scoped timeout override payload field.
...
This mirrors the `cli-surface` capability's `Send Timeout Override Flags by
Transport` requirement, and deletes the same per-coder timeout keys this
requirement previously enumerated — `[coders.<id>.acp].prime-timeout-ms`,
...
The pre-existing `quiescence_timeout_ms` payload field was retired by the
`tmux-wedge-detection` proposal; the pre-existing `acp_turn_timeout_ms` payload
field was retired by `establish-delivery-commit-contract`. Both fields are
rejected by the MCP server as unknown. Alpha defaults apply: the rejection is a
generic unknown-field error; the server is NOT required to name a replacement.
Operators who hit the rejection consult the changelog.

`send` SHALL fail fast on a transport-incompatible timeout field with
`validation_invalid_timeout_field_for_transport`. With no transport-scoped
timeout override fields, this validation class is reserved for a future
per-call override, if one is ever reintroduced.
...
- **WHEN** `send` request payload includes `quiescence_timeout_ms` (a field
  that does not exist)
- **THEN** the tool rejects the field as unknown

#### Scenario: Reject retired ACP timeout payload field

- **WHEN** `send` request payload includes `acp_turn_timeout_ms` (a field
  that does not exist)
- **THEN** the tool rejects the field as unknown

#### Scenario: No payload field bounds how long a delivery waits

- **WHEN** a caller wants to change how long a delivery waits for a target to
  become ready
- **THEN** no payload field and no configuration key offers that control,
  because the wait is unbounded by design
- **AND** `send` accepts no per-call timeout override of any kind
...
- **AND** `change.command` is constrained to `psk` and `scope`
...
equivalent SHALL be `agentmux change scope <principal_id> --scope SCOPE`, with
existing runtime-root, requester-selection and JSON-output conventions. Relay
`ChangeScope` SHALL carry those two fields and dispatch relay-wide, without
requiring a routing bundle.
...
shape and grammar before relay submission; the relay SHALL independently validate
and enforce `Peer Scope Administration Authorization`. Invalid input SHALL
produce `validation_invalid_params` identifying the field. A missing target
record SHALL produce `validation_unknown_principal` after authorization. Only a
registered relay principal can be updated by this command.
...
The system SHALL expose a meta-tool `link` that installs a peer credential into
the connected relay's relay-owned slot. `link` SHALL require `command="peer"`.
...
relay-owned directories with mode 0700. Installation SHALL classify at commit
`new.peer=all`; slot present with byte-identical content SHALL require
`new.peer=all` OR `change.psk=all`; slot present with different content SHALL
require `change.psk=all`. `home` SHALL be insufficient. Installation with
...
argument in request memory and structured payloads. It SHALL NEVER be written to
logs, snippets, or diagnostics, and every `link` response SHALL omit it,
...
- **WHEN** a caller installs for an alias with no registered `<alias>@RELAY`
  relay principal
