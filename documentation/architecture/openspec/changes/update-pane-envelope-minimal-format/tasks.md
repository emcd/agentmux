...
- [ ] 1.4 Preserve boundary start/end framing and enforce deterministic boundary
      token derivation from first boundary line.
- [ ] 1.5 Preserve human-addressing headers (`From`, `To`, optional `Cc`,
      optional `Subject`) plus `Message-Id` and `Date`.
- [ ] 1.6 Ensure machine metadata required for routing/audit is emitted via
