...
- [x] 1.1 Enforce the shell-literal-safe affix grammar over the
...
- [x] 2.2 Implement precedence (override > applicable rule >
...
- [x] 2.3 Substitute `{{project-name}}` raw in tmux/pty rendering;
  reject unknown `{{project-name}}`-adjacent misspellings through
  the general classifier.
- [x] 2.4 Controls for every precedence tier, both rule values,
...
- [x] 3.1 Document composed forms, precedence, grammar, and the
  standalone-to-composed migration in the maintainer guide
  `coders.toml` and bundle sections.
- [x] 3.2 Record Added/Changed entries under CHANGELOG Unreleased.
- [x] 3.3 Validate `openspec validate --all --strict`, delta audit,
