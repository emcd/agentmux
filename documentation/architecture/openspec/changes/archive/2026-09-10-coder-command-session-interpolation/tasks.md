...
- [x] 1.1 Accept only the three double-brace names in classification;
...
- [x] 2.2 Migrate every repository-owned `{coder-session-id}` fixture
  to the double-brace form; full nextest run plus fmt/clippy per lane
  workflow.
- [x] 2.3 Brace-exact proof: zero single-brace hits in positive
...
- [x] 3.1 Document the three double-brace names in the maintainer
