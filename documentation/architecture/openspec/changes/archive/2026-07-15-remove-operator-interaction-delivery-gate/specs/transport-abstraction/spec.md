> **Delta base.** These `MODIFIED` requirements are reproduced from the live
> spec **after** `add-wedge-detection-busy-state` archived (master `882283b`) —
> i.e. with the `Busy` pre-classification, its Branch-ordering contract, its
> Scope clarification, and its four Busy scenarios all present. A `MODIFIED`
> delta replaces the requirement whole, so every one of those is carried
> forward verbatim here; the ONLY substantive edits are the removal of the
> operator-interaction gate. Because of that whole-requirement replacement, this
> change MUST NOT be authored or archived against the pre-archive spec, and it is
> already sequenced after busy-state (which is now archived). See `tasks.md` §0.
>
> This delta also repairs an orphaned bullet fragment that the busy-state archive
> left dangling at the end of the requirement (the tail of the "Busy short-circuit
> resets wedge counter" scenario had been displaced past two later scenarios); it
> is reattached to its scenario here.
>
> **Not touched by this change:** the ACP transport's `pending_choice_outcome`
> pause (`ACP Prime Timeout Envelope Field Consumption` requirement) is a distinct
> mechanism — an operator *decision* on a choice/approval that genuinely blocks
> the turn — not a rendering artifact like tmux copy-mode. It is out of scope and
