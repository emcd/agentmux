...
- [x] 1.1 Sync `runtime-bootstrap` — `Relay Configuration File` gains the
...
- [x] 1.2 Sync `addressing-routing` — `Session Type Taxonomy` reads five types
      with the `Pty` row and the new derivation scenario
- [x] 1.3 Sync `look-and-stream-events` — `ACP Look Snapshot Contract` no longer
      describes `take_replay_entries` as available, and its eleven `\'`
      sequences are corrected
- [x] 1.4 Sync `relay-routing-layer` — `Cross-Relay Target Ingress Filter` no
...
- [x] 1.5 Sync `tui-surface` — `Initial TUI Workflow Coverage` cites
      `look-and-stream-events` rather than the archived change
- [x] 1.6 Sync `cli-surface` — `CLI raww actor identity resolution` reads
      `users.toml`
- [x] 1.7 Sync `authorization-scope` — `UI Request-Path Sender Validation` reads
      `users.toml`

## 2. Direct prose edits (no delta mechanism governs these)

- [x] 2.1 `specs/cli-surface/spec.md` Purpose preamble: replace the `tui.toml`
...
- [x] 2.2 `specs/session-relay/spec.md` hub preamble: correct the
      "session type taxonomy (tmux/acp/ui/pubsub)" echo to include `pty`

## 3. Verify

- [x] 3.1 `scripts/verify-openspec-deltas.py update-stale-live-spec-statements`
...
- [x] 3.3 Confirm no `tui.toml` reference remains anywhere under
      `openspec/specs/`
- [x] 3.4 Confirm no live spec still cites an archived change as normative
      authority for these requirements
- [x] 3.5 Confirm the four-session-type claim survives nowhere: sweep
      `openspec/specs/` for "four session types" and for
      taxonomy lists omitting `pty`
- [x] 3.6 Spot-check that each synced requirement retains every scenario it had
      before, since `openspec validate` cannot see retention

## 4. Close out

- [x] 4.1 Mark `todos/openspec/3` complete
- [x] 4.2 Mark `todos/general/34` complete
- [x] 4.3 File the residual finding: `todos/openspec/4` (allowed-scope cap) and
