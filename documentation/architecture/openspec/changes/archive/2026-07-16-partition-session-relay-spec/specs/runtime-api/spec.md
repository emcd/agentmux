...
The 10 ADDED requirements below establish the new runtime-api partition. Their authoritative content lives in `embeddable-runtime-api`; the partition-session-relay-spec change copies them verbatim into both the live `openspec/specs/runtime-api/spec.md` (via `populate_runtime_api.py`) and this delta spec for completeness. The change does NOT alter the requirement content -- it only relocates where the requirement lives.
