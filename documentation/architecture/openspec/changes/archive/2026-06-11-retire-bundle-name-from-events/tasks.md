...
- [x] 1.1 Audit every `RelayStreamEvent { ... }` construction site in
...
- [x] 1.2 Remove `bundle_name` field from `RelayStreamEvent` in both
...
- [x] 4.2 Remove `bundle_name` destructure and re-emit from the three
      handler files identified in 4.1; remove from success inscriptions
- [x] 4.3 Update integration test fixtures in `tests/integration/mcp/`
...
- [x] 5.1 After implementation, run `openspec validate --strict` and resolve
      any issues
- [x] 5.2 Archive `retire-bundle-name-from-events` with spec updates to
