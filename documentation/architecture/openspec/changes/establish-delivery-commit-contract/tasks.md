...
- [ ] Move Pty's `startup` off the async worker. It spawns a child inline in `build_worker_transport`, which runs on the delivery runtime; the same restructuring the tmux observation needed. Deferred while the Pty construction path is still placeholder pending per-coder config parsing
