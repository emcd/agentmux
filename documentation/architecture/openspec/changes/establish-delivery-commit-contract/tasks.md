...
- [ ] Implement the relay-owned guard: keyed on `(batch, member, attempt)` at authorization, atomically bound to `PackingUnit ID` at partition — creation at authorization is done, and the binding is now a real `PackingUnit ID` recorded before the first target-side effect rather than a boolean standing in for one. What remains is the partition itself. All three coder transports now report their own — a budget group is one unit, declared before the write that carries it — so a multi-member unit is real rather than hypothetical. Pty stays one member per unit by commitment, not by omission
