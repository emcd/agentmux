...
- [ ] 1.1 Remove `bundle_name: Option<String>` from `RelayRequest::PermissionResolve`
      (`src/relay/contract.rs`). Also remove the corresponding destructure arm in
      `src/relay/handlers/dispatch.rs` (around line 58–72).
- [ ] 1.2 Remove `bundle_name: Option<String>` from `RelayRequest::PermissionList`
      (`src/relay/contract.rs`). Also remove the corresponding destructure arm in
      `src/relay/handlers/dispatch.rs` (around line 77–85).
- [ ] 1.3 Remove `bundle_name: Option<String>` from `RelayRequest::IdentityIntrospect`
...
- [ ] 2.1 In `handle_permission_decision` (`src/relay/handlers/permissions.rs`),
      ...
      struct.
- [ ] 2.2 In `validate_permission_decision_request`, remove the `request_bundle_name`
...
- [ ] 2.3 In `handle_permission_list`, remove the `request_bundle_name` parameter
...
- [ ] 3.1 In `handle_identity_introspect` (`src/relay/handlers/identity.rs`),
...
- [ ] 3.2 Add a pre-check: if `target_session` is not a qualified principal id,
...
- [ ] 4.1 `src/mcp/server/handlers/grant.rs` — remove `bundle_name: None` from
      ...
      compile-forced.)
- [ ] 4.2 `src/tui/state/compose/permissions.rs:139` — remove `bundle_name: None`
...
- [ ] 5.1 Update `tests/unit/relay_stream/identity.rs`: drop the `bundle_name:
      ...
      the new test in 5.2.
- [ ] 5.2 Add a test asserting `validation_invalid_params` when
      `IdentityIntrospect` is called with a bare (unqualified) `target_session`.
- [ ] 5.3 Confirm that no test asserts `validation_cross_bundle_unsupported`
