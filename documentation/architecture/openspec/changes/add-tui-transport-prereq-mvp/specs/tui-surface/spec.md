...
The MVP SHALL additionally define one structured relay-backed transport contract
for TUI inbound message and delivery-outcome updates needed by history and
status views.
...
- **THEN** the TUI supports recipient selection, send, look, and update
  workflows
- **AND** inbound message and delivery outcome updates are represented with one
  canonical structured transport contract
...
2. `tui.toml` configured default sender when provided
3. runtime association-derived sender identity
4. fail-fast validation error when unresolved

TUI runtime SHALL use the resolved sender identity consistently for relay-backed
operations in that TUI process.

#### Scenario: Prefer CLI sender over configured default

- **WHEN** operator starts TUI with `--sender` and `tui.toml` also defines a
  sender
...
- **WHEN** relay transport is unavailable during TUI update retrieval
...
- **WHEN** TUI update retrieval request attempts bundle scope outside associated
  context
