...
- [ ] 1.1 Sync `authorization-scope` — `Uniform Cross-Bundle Authorization
...
- [ ] 1.2 Sync `relay-routing-layer` — `Authorization Stage` keeps the rule
...
- [ ] 2.1 `specs/relay-routing-layer/spec.md` Purpose preamble: remove the
      parenthetical naming the policy schema's per-capability allowed-scope set
      as the sole authority for cross-bundle reach
- [ ] 2.2 `src/relay/routing.rs` module documentation: remove the sentence
...
- [ ] 3.1 `scripts/verify-openspec-deltas.py remove-policy-schema-scope-cap`
...
- [ ] 3.3 Confirm no spec or source comment still describes a per-capability
      allowed-scope cap, a policy-schema cap, or a capability being unreachable
      cross-bundle until a schema is widened
- [ ] 3.4 Confirm the guard comment at `src/relay/authorization/loading.rs`
      still stands and now agrees with every specification statement about
      control scopes
- [ ] 3.5 Confirm no remaining scenario has a precondition that no
      configuration can establish

## 4. Close out

- [ ] 4.1 Mark `todos/openspec/4` complete
- [ ] 4.2 Confirm `todos/openspec/5` (distribute verb authorizability) is
