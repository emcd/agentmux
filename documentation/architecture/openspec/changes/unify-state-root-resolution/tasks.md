...
- [ ] 1.1 Add `AGENTMUX_STATE_DIRECTORY` as the environment tier for the state
...
- [ ] 1.2 Remove the `cfg!(debug_assertions)` branches from `resolve_state_root`
  and `resolve_inscriptions_root`
- [ ] 1.3 Confirm the inscriptions root still defaults to
  `<state_root>/inscriptions` and therefore follows the state root without
  separate selection
- [ ] 1.4 Remove `repository_root` from `RuntimeRootOverrides`
- [ ] 1.5 Normalize the resolved state root to a non-empty absolute path,
...
- [ ] 1.6 Bind and connect Agentmux-owned sockets through their parent directory
  ...
  worst case
- [ ] 1.7 Settle the tmux socket, which is the longest path at 96 bytes and is
...
- [ ] 1.8 Report a socket path-length failure as a structured error naming the
  limit and the offending path, rather than the raw bind errno an operator sees
  today

## 2. Deleting the Git provenance

- [ ] 2.1 Delete `repository_checkout_root`, `git_common_directory`,
...
- [ ] 2.2 Remove `debug_repository_state_root` and
  `debug_repository_inscriptions_root`
- [ ] 2.3 Remove `--repository-root` and stop threading it through
  `commands/shared.rs` and `commands/host/relay.rs`
- [ ] 2.4 Confirm no `git` subprocess remains at startup, and drop the
  `git`-on-`PATH` expectation from prerequisites documentation if stated there

## 3. Propagation

- [ ] 3.1 Inject `AGENTMUX_STATE_DIRECTORY` at spawn, from the relay's normalized
  ...
  them
- [ ] 3.2 Split the two enumerations `BringUpContext::VARIABLE_NAMES` currently
...
- [ ] 3.3 Point the test-harness sanitizer
...
- [ ] 5.3 Prove the state-root tier order, including that `--state-directory`
  outranks `AGENTMUX_STATE_DIRECTORY` and that a blank environment value is
  absent rather than empty
- [ ] 5.4 Prove a debug build and a release build resolve identical state and
