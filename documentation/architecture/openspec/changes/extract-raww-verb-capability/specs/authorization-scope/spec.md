...
**Reason**: Relocated verbatim to the new `raww` capability. `authorization-scope`
retains the verb-independent model -- the scope ladder, evaluation order, denial
schema, and the uniform cross-bundle tiers -- and no longer carries a mapping for
one particular verb.
