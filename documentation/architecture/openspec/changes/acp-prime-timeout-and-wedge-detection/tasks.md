...
- [ ] 1.2 ADD the ACP Prime Timeout Envelope Field Consumption
      ...
      on archive)
- [ ] 1.3 MODIFY the Send Timeout Override Flags by Transport
...
- [ ] 2.1 Rename `AcpTargetConfiguration.turn_timeout_ms` →
      `AcpTargetConfiguration.prime_timeout_ms` in
      `src/configuration/types.rs` (type declaration).
- [ ] 2.2 Rename the field in `src/configuration/raw.rs` (the
      `RawAcpTarget` raw loader and `AcpTarget` intermediate
      loader).
- [ ] 2.3 Rename the field in `src/configuration/targets.rs`
...
- [ ] 2.4 Confirm the rename compiles and that no other
...
- [ ] 2.8 Update `src/relay/delivery/quiescence.rs::QuiescenceOptions`
...
- [ ] 3.1 Confirm `DeliveryEnvelope.prime_timeout_ms: Option<u64>`
      ...
      in this proposal.
- [ ] 3.2 Confirm the field replaces the legacy
...
- [ ] 4.1 In `src/acp/transport.rs::acp_delivery_task` /
...
- [ ] 4.2 Track the prime timer anchor: start time when first
      `wait_for_prompt_complete` is called for the flush group.
      Do NOT reset on coalesce-during-wait.
- [ ] 4.3 On each `wait_for_prompt_complete` poll, check whether
...
- [ ] 4.4 Confirm `acp_delivery_task` does NOT inject further
...
- [ ] 4.5 Confirm the per-turn readiness transition on
      prime-timer fire is `Busy -> Unavailable` (matching
      `Decision 7` in `design.md`).
- [ ] 4.6 Confirm the prime timer starts only after
...
- [ ] 5.1 Add `acp_prime_timeout_fires_after_configured_window`
...
- [ ] 5.2 Add `acp_prime_timer_does_not_reset_on_coalesce` to
...
- [ ] 5.3 Add `acp_prime_timer_does_not_fire_during_pending_choice`
      to `tests/integration/acp/lifecycle.rs`. Set
...
- [ ] 5.4 Add `acp_prime_timeout_default_unbounded` to
...
- [ ] 5.5 Add a fixture-based test for the operator knob rename:
      ...
      successfully.
- [ ] 5.6 Add a CLI rejection test: `agentmux send
      --acp-turn-timeout-ms 1000 ...` fails at the parser as an
      unknown flag. The test asserts the rejection happens; it
      does NOT assert the error message names a replacement (alpha
      defaults — sharp break).
- [ ] 5.7 Add an MCP rejection test: a `send` payload that
      includes `acp_turn_timeout_ms` is rejected as an unknown
      field. The test asserts the rejection happens; it does NOT
      assert the error message names a replacement (alpha
      defaults — sharp break).
- [ ] 5.8 Add unit tests for the prime-timer anchor calculation
...
- [ ] 5.9 Confirm existing `#[ignore]`d ACP tests
...
- [ ] 6.1 Emit `delivery_prime_timeout` inscription with
...
- [ ] 6.2 Confirm `delivery_prime_timeout` is emitted on the ACP
      path with the same field shape as the Tmux path (no
      `acp_`-prefixed fields).

## 7. Documentation

- [ ] 7.1 Update `src/acp/README.md` (if present) to describe
...
- [ ] 7.2 Update operator-facing bundle config docs to describe
...
- [ ] 7.3 Add a "prime timeout suppresses during pending choice"
      note to the ACP transport docs (cross-reference the
      non-expiring choice pending lifecycle contract).
- [ ] 7.4 Document the per-call override retirement in the
...
- [ ] 7.5 Record the wedge detection deferral in
      `documentation/architecture/decisions/` (if such a
      directory exists) — the rationale is in `design.md`
      Decision 3; the archive note preserves it for the next
      wedge proposal.
...
- [ ] 8.4 `openspec validate acp-prime-timeout-and-wedge-detection
      --strict` passes
- [ ] 8.5 Pre-commit hooks pass
- [ ] 8.6 Confirm the three `#[ignore]`d ACP tests remain
