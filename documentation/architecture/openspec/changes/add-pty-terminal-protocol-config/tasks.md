...
- [ ] 1.2 Add `term_protocol: Option<TermProtocol>` to
      `RawPtyTarget` in `src/configuration/raw.rs` (kebab-case
      TOML key `term-protocol`, `#[serde(default)]`).
- [ ] 1.3 Add `term_protocol: Option<TermProtocol>` to validated
      `PtyTarget` in `src/configuration/raw.rs`.
- [ ] 1.4 Add `term_protocol: TermProtocol` to
...
- [ ] 2.1 No new validator code is required. Unknown
...
- [ ] 2.2 The existing per-coder mutual-exclusion validator
      between `[coders.<id>.tmux]` / `[coders.<id>.acp]` /
      `[coders.<id>.pty]` is unchanged.

## 3. Transport-side wiring

- [ ] 3.1 Add `term_protocol: TermProtocol` (clone of validated
      type) to `pty::transport::PtyTargetConfiguration` in
      `src/pty/transport.rs`.
- [ ] 3.2 Update `src/relay/delivery/dispatch/worker.rs` to
...
- [ ] 4.1 Add a `term_protocol_round_trips_to_env_var` test
      ...
      - asserts `TermProtocol::default().as_env_var() ==
        "xterm-256color"`.
- [ ] 4.2 Add a `term_protocol_propagates_to_child_command` test
...
- [ ] 5.1 Update `README.md` Pty transport section with the new
...
- [ ] 5.2 Update `data/configuration/coders.toml` with a sample
      `[coders.<id>.pty]` block (commented as illustrative) so
      operators see the available values.

## 6. Validation

- [ ] 6.1 `cargo test --lib` and `cargo test --tests` pass
      with no regressions.
- [ ] 6.2 `cargo clippy --all-targets --no-deps` is silent.
- [ ] 6.3 `cargo fmt --check` is silent.
- [ ] 6.4 `openspec validate add-pty-terminal-protocol-config
      --strict` passes.
