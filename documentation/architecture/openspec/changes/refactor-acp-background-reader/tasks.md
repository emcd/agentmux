...
- [ ] 3.1 Add `pending_requests: Arc<Mutex<HashMap<u64, oneshot::Sender<serde_json::Value>>>>` to reader state
- [ ] 3.2 Expose `register_pending(request_id) -> oneshot::Receiver<Value>` for non-prompt callers
- [ ] 3.3 Background reader looks up `id` in registry on any JSON-RPC response and sends on the oneshot channel
- [ ] 3.4 On reader exit, drain registry and send `Err` on all pending receivers

## 4. Background reader thread

- [ ] 4.1 Spawn reader thread in `AcpStdioClient` after child process is started
- [ ] 4.2 Reader loop: `read_line` on `BufReader<ChildStdout>` → parse JSON-RPC message
- [ ] 4.3 Dispatch by message type:
...
- [ ] 4.4 On reader loop exit (EOF or I/O error): transition worker state to `Unavailable`; drain pending-request registry with error; close turn-complete sender if set
- [ ] 4.5 Store reader `JoinHandle` in `AcpStdioClient` for lifecycle management
