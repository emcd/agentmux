...
### Requirement: ACP Permission Event Payload Contract

Relay SHALL emit `permission.requested` stream events with a stable payload
shape when the background reader dispatches a `session/request_permission`
notification.

`permission.requested` event payload SHALL include:

- `permission_id` (string) — relay-assigned stable identifier for this request
- `target_session` (string) — ACP session the permission is being requested for
- `request_type` (string) — permission type (pass-through from ACP message)
- `request_payload` (object) — ACP permission request body (pass-through)
- `requested_at` (RFC 3339 string)

Approve/deny relay request shape SHALL include:

- `permission_id` (string) — matches the `permission.requested` event
- `decision` (string enum: `allow` | `deny`)
- `target_session` (string)

Relay SHALL write the ACP JSON-RPC response to ACP stdin via the shared stdin
writer immediately when a decision is received.

This payload shape is published early to unblock MCP consumer surface design
(`todos/mcp/24`) in parallel with the relay implementation.

#### Scenario: permission.requested event carries stable permission_id

- **WHEN** background reader dispatches a `session/request_permission` notification
- **THEN** relay emits a `permission.requested` stream event
- **AND** event payload includes a `permission_id` that can be used to approve
  or deny the request

#### Scenario: Approve decision writes JSON-RPC response to ACP stdin

- **WHEN** relay receives an `allow` decision for `permission_id`
- **THEN** relay writes the JSON-RPC response (approved) to ACP stdin via
  the shared stdin writer
- **AND** ACP child process receives the response and continues processing

#### Scenario: Deny decision writes JSON-RPC response to ACP stdin

- **WHEN** relay receives a `deny` decision for `permission_id`
- **THEN** relay writes the JSON-RPC response (denied) to ACP stdin via
  the shared stdin writer
- **AND** ACP child process receives the response
