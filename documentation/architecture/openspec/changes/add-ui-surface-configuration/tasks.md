...
- [ ] 1.1 Add `UiConfiguration { default_bundle: Option<String> }` in
...
- [ ] 1.2 Add `load_ui_configuration(configuration_root)` mirroring
...
- [ ] 1.3 Remove `default_bundle` from `TuiConfiguration` (delete the field
      outright; no rejection logic — `deny_unknown_fields` covers a stray key).
- [ ] 1.4 Public re-exports for `UiConfiguration` / `load_ui_configuration`
      alongside the existing TUI configuration exports.

## 2. Runtime resolution

- [ ] 2.1 In `src/runtime/tui_session.rs`, load `ui.toml` alongside the TUI
...
- [ ] 2.2 Keep `default-session` resolution sourced from `users.toml`
      (`TuiConfiguration`); confirm the debug/testing `overrides/users.toml`
      precedence is unchanged and applies only to `users.toml`.
- [ ] 2.3 Update the fail-fast error text that names the source file
...
- [ ] 2.4 Wire `load_ui_configuration` into `agentmux check configuration`
...
- [ ] 3.1 Remove the commented `default-bundle` from
      `data/configuration/users.toml`.
- [ ] 3.2 Add `data/configuration/ui.toml` with a commented `default-bundle`
      example and a short header comment.

## 4. Documentation

- [ ] 4.1 Update `src/runtime/README.md` and `src/tui/README.md` bundle-
      precedence prose to source `default-bundle` from `ui.toml`.
- [ ] 4.2 Update `documentation/usage/` where `default-bundle`/`users.toml`
      bundle defaults are described.

## 5. Tests

- [ ] 5.1 Unit: `ui.toml` load — valid `default-bundle`, absent file → no
      default, malformed → structured error.
- [ ] 5.2 Behavioral: TUI/CLI bundle resolution sources `default-bundle` from
...
- [ ] 5.3 Behavioral: `agentmux check configuration` exits non-zero and names
      the file when `ui.toml` is malformed; exits zero when it is valid or
      absent.
- [ ] 5.4 Behavioral: the debug `overrides/users.toml` override affects identity
