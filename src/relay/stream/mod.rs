...
use super::identity::{
    PrincipalType, canonical_session_id, classify_principal_id, split_principal_id,
};
use super::{RelayRequest, RelayResponse};

/// Free-fn helpers that tear down registry entries (or fan revocation events
/// out to trusted hosts). Extracted to a sibling module so the registry
/// state-machine code below can stay focused on stream registration and
/// frame dispatch; re-exported here so the existing external callers in
/// `relay::watcher` and `relay::handlers::identity` continue to invoke the
/// helpers via `super::stream::evict_*` without an extra `eviction::` step.
pub(super) mod eviction;
pub(crate) use self::eviction::{
    evict_streams_for_bundle, notify_trusted_hosts_of_revocation, revoke_streams_for_identity,
};
