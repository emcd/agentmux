...
mod tests {
    ...
    use super::{
        BundleRuntimePaths, RuntimeRootOverrides, RuntimeRoots, configuration_root_from_sources,
        debug_repository_state_root, ensure_bundle_runtime_directory, state_root_from_sources,
    };

    #[test]
    fn uses_xdg_configuration_root_when_present() {
        let root = configuration_root_from_sources(
            Some(std::path::Path::new("/xdg/config")),
            std::path::Path::new("/home/example"),
        );
        assert_eq!(root, std::path::Path::new("/xdg/config/tmuxmux"));
    }

    #[test]
    fn falls_back_to_home_configuration_root() {
        let root = configuration_root_from_sources(None, std::path::Path::new("/home/example"));
        assert_eq!(root, std::path::Path::new("/home/example/.config/tmuxmux"));
    }

    #[test]
    fn uses_xdg_state_root_when_present() {
        let root = state_root_from_sources(
            Some(std::path::Path::new("/xdg/state")),
            std::path::Path::new("/home/example"),
        );
        assert_eq!(root, std::path::Path::new("/xdg/state/tmuxmux"));
    }

    #[test]
    fn falls_back_to_home_state_root() {
        let root = state_root_from_sources(None, std::path::Path::new("/home/example"));
        assert_eq!(
            root,
            std::path::Path::new("/home/example/.local/state/tmuxmux")
        );
    }
    ...
    fn resolves_bundle_runtime_paths() {
        let resolved =
            BundleRuntimePaths::resolve(std::path::Path::new("/state/root"), "party-alpha")
