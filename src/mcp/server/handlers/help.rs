...
impl McpServer {
    #[tool(
        name = "help",
        description = "Return tool/command help and JSON schemas. Query omitted or `agentmux` for the tool list; `list`, `updown`, `new`, or `change` for a meta-tool's command list; or `list.principals`/`list.decisions`/`send`/`look`/`raww`/`choose`/`updown.up`/`updown.down`/`new.peer`/`change.psk`/`change.scope` for exact arg schemas."
    )]
    async fn tool_help(
        &self,
